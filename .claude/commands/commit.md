---
name: commit
description: Stage changes, run a local review pass, draft a Conventional Commit message from the diff, and commit. Refuses to run on master/main.
...
**Refuse if HEAD is `master` or `main`** — tell the user to run `/branch <name>` first.

Steps:
1. Run `git status` and `git branch --show-current` in parallel.
2. If branch is `master` or `main`, stop and instruct the user to create a feature branch with `/branch`.
